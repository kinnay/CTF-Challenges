...
class Twister(object):
    def __init__(self, offset=0):
        ...
        pos = 0
        for i in range(N):
            self.tw.append([frozenset((i,)) for i in range(pos, pos+32)])
            pos += 32
...
if __name__ == "__main__":
    ...
    for byte in range(10000):
        ...
        if byte % 625 == 0:
            ...
            for p, bits in enumerate(twister.tw):
                for bn, bit in enumerate(bits):
                    #assert sum(map(getbit, bit)) & 1 == getbit(bn + p * 32, now_state), (p, bn)
                    if sum(map(getbit, bit)) & 1 != getbit(bn + p * 32, now_state):
                        print(p, bn, bit)
                        falsch = True
        ...
        for bit, d in enumerate(w):
            assert sum(map(getbit, d)) & 1 == (v >> (31 - bit)) & 1, byte
        v = random.getrandbits(32)
    print("OK")
